...
%token EQUAL EXCEPT EXEC FINALLY FLTNUMBER FOR FROM GLOBAL GREATER GREATEREQUAL 
%token GRLT IF IMPORT IN INDENT INTNUMBER IS LAMBDA LBRACE LEFTSHIFT LEFTSHIFTEQUAL
%token LESS LESSEQUAL LPAR LSQB MINEQUAL MINUS NAME NEWLINE NOT NOTEQUAL 
...
%type<intNumber> INTNUMBER MINUS PLUS TILDE pick_unop pick_PLUS_MINUS pick_multop
	...
	: pick_unop factor
	  {
	    ...
	    {
	      $$ = new AddBinaryNode(new IntLiteral(0), $2);
	      pool.add($$);
	    }
	  }
	| power
...
power // Used in: factor
	: atom star_trailer DOUBLESTAR factor
	  {
	    int i1 = atoi($1->eval()->getVal());
	    int i2 = atoi($4->eval()->getVal());
	    float f1 = atof($1->eval()->getVal());
	    float f2 = atof($4->eval()->getVal());
	    if(i1 == f1 and i2==f2)
	      $$ = new IntLiteral(pow(i1, i2));
	    else
	      $$ = new FloatLiteral(pow(f1, f2));
	  }
	| atom star_trailer
	;
